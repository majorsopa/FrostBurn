...
public class ConfigManager {
    ...
    public static void load(String name) {
        try {
            ...
            for (Module module : ModuleManager.modules) {
                if (Boolean.parseBoolean(config.getProperty(module.getName() + ".enabled")) != module.isEnabled())
                    module.setEnabled(Boolean.parseBoolean(config.getProperty(module.getName() + ".enabled")));
                ...
                for (Setting setting : module.settings) {
                    ...
                    if (value != null) {
                        switch (setting.getType()) {
                            case FLOAT:
                                if ((float) setting.getMin() >= Float.parseFloat(value) || (float) setting.getMax() <= Float.parseFloat(value))
                                    setting.setValue(Float.parseFloat(value));
                                break;
                            case INTEGER:
                                if ((int) setting.getMin() >= Integer.parseInt(value) || (int) setting.getMax() <= Integer.parseInt(value))
                            ...
                            case BOOLEAN:
                                ...
                                break;
                            case STRING:
                                if (setting.getOptions().contains(value) || setting.getOptions().contains(value))
