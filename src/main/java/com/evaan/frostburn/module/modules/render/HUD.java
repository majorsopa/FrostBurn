...
public class HUD extends Module {
    ...
    public void onRender1(MatrixStack matrices) {
        ...
        if (greeter.getValue()) {
            ...
            if (hour >= 0 && hour < 12)
                mc.textRenderer.drawWithShadow(matrices, "Good morning " + mc.player.getName().asString() + " :^)", (mc.getWindow().getWidth() / 2) - (mc.textRenderer.getWidth("Good morning " + mc.player.getName() + ":^)") / 2), 2, rgb);
            else if (hour >= 12 && hour < 16)
                mc.textRenderer.drawWithShadow(matrices, "Good afternoon " + mc.player.getName().asString() + " :^)", (mc.getWindow().getWidth() / 2) - (mc.textRenderer.getWidth("Good afternoon " + mc.player.getName() + ":^)") / 2), 2, rgb);
            else if (hour >= 16 && hour < 24)
                mc.textRenderer.drawWithShadow(matrices, "Good evening " + mc.player.getName().asString() + " :^)", (mc.getWindow().getWidth() / 2) - (mc.textRenderer.getWidth("Good evening " + mc.player.getName() + ":^)") / 2), 2, rgb);
        }
        ...
        if (coords.getValue()) {
            ...
            else if (mc.world.getRegistryKey().getValue().getPath().equalsIgnoreCase("overworld"))
                mc.textRenderer.drawWithShadow(matrices, format.format(mc.player.getX()) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getY()) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getZ()) + Formatting.WHITE + " [" + Formatting.RESET + format.format(mc.player.getX() * 8) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getY()) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getZ() * 8) + Formatting.WHITE + "]", 2, mc.getWindow().getHeight() - mc.textRenderer.fontHeight - 2, rgb);
            else
                mc.textRenderer.drawWithShadow(matrices, format.format(mc.player.getX()) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getY()) + Formatting.WHITE + ", " + Formatting.RESET + format.format(mc.player.getZ()), 2, mc.getWindow().getHeight() - mc.textRenderer.fontHeight - 2, rgb);
        }
    }
}